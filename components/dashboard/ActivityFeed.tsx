      ...
      {transactions.map(tx => (
        <div key={tx.id} className="flex items-center gap-3 p-2 rounded-xl hover:bg-earth-50 transition-colors">
          <div className={`flex-shrink-0 w-9 h-9 rounded-xl flex items-center justify-center
            ${tx.direction === 'in' ? 'bg-forest-400/10 text-forest-500' : 'bg-earth-100 text-earth-300'}`}>
            {TX_ICONS[tx.type] || <ArrowUpRight className="w-4 h-4" />}
          </div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-ink-100 truncate">{TX_LABELS[tx.type] || tx.type}</p>
