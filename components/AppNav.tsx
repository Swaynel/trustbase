...
import {
  LayoutDashboard, Users, ShoppingBag, Landmark, Vote,
  ArrowLeftRight, UserCircle, LogOut, Menu, X, Shield
} from 'lucide-react'
        ...
        {NAV_ITEMS.map(item => {
          ...
          return (
            ...
            >
              ...
              {item.label}
              {locked && (
                <span className="ml-auto text-[10px] text-earth-300 font-mono">Lv{item.minLevel}</span>
              )}
            </Link>
          )
        })}
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 h-14 bg-white border-b border-earth-200 flex items-center justify-between px-4 z-30">
        ...
        <button onClick={() => setMobileOpen(v => !v)} className="p-2 rounded-lg hover:bg-earth-50">
